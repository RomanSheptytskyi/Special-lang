...
class TestConsoleApp(unittest.TestCase):
    ...
    def test_show_users(self, mock_print):
        ...
        self.app.history_manager.log.assert_called_with("Показати користувачів", "Виведено всіх користувачів.")
    ...
    def test_show_posts(self, mock_print):
        ...
        # Перевіряємо, що викликано правильно виведення на екран через print
        mock_print.assert_called_with(
            '+----+------------+---------------------------+------------------+\n| ID | Заголовок  | Тіло                      | ID користувача   |\n+----+------------+---------------------------+------------------+\n| 1  | Post 1     | Content of post 1         | 1                |\n+----+------------+---------------------------+------------------+'
        )
        self.app.history_manager.log.assert_called_with("Показати пости", "Виведено всі пости.")
    ...
    @patch('builtins.input', side_effect=["New Post", "This is a new post", "1"])
    def test_add_post(self, mock_input):
        # Тестуємо метод add_post
        self.app.add_post()
        self.app.adder.add_post.assert_called_with("New Post", "This is a new post", 1)
